...
# Прежде мы передавали обучающие примеры в сеть по одному, корректируя веса после каждого из них.
# Теперь сети будет передаваться сразу по 100 примеров, усредняя корректирующие значения для весов по всем 100 примерам.
batch_size = 100
...
# Обратите внимание, что точность на контрольных данных изменяется более плавно, чем в предыдущих примерах.
# Этот эффект обусловлен усреднением корректирующих воздействий на весовые коэффициенты в процессе обучения.
# Как оказывается, отдельные обучающие примеры несут в себе очень много шума,
# в смысле корректирующих значений, которые они генерируют.
# То есть усреднение делает процесс обучения более плавным и быстрым.

# Обратите внимание, что здесь значение коэффициента alpha в 20 раз больше, чем в предыдущих примерах.
# В данном случае мы смогли увеличить его из-за того, что при усреднении данных отклонение значительно меньше,
# чем без него.
